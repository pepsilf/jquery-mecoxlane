                ...
                $.ajax({
                    ...
                    if (back.status) {
                        ...
                        for (var i in back.data) {
                            var moneys = back.data[i].g_price.split("￥")[1] * back.data[i].g_nums
                            $moneys += moneys;
                            $nums = back.data[i].g_nums - 0 + $nums;
                            $str = $str + `<tr>
                                    <th class="selector"><input type="checkbox" class="oneChecked"/></th>
                                    <th>
                                        <div>
                                            <img src="${back.data[i].g_src}">
                                            <span>${back.data[i].g_name}</span>
                                            <p>商品编号：${back.data[i].g_num}</p>
                                        </div>
                                    </th>
                                    <th>${back.data[i].g_price}</th>
                                    <th class="nums"><span>-</span><input type="text" value="${back.data[i].g_nums}"><span>+</span></th>
                                    <th>￥${moneys}</th>
                                    <th class="del" gid='${back.data[i].g_id}' uname='${back.data[i].u_name}'><span>X删除</span></th>
                                </tr>`
                        }
                    }
                    $("#tab").html($str)
